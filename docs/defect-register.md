...
| D-010 | Affiliate links cannot actually monetize: `AMAZON_AFFILIATE_TAG` / `EBAY_CAMPAIGN_ID` are read at render time in client components but are not `NEXT_PUBLIC_*`, so they are always undefined in the browser — setting them in Vercel would silently do nothing | Major (business) | Env-var access pattern assumed server rendering; link builders run client-side | Never exercised: no affiliate program is enrolled yet, so the gap was invisible | Documented in affiliate-config comments + `project_affiliate_income_primary` memory; Footer/disclosure now derive from the same config so the honest state always displays. Fix (NEXT_PUBLIC variants or server-side link generation) deliberately deferred until program enrollment — needs a product decision on approach | **Open — pre-monetization blocker** (does not affect beta content correctness) |
