...
| 2026-05-08 | Homepage headline locked: "Audio XX helps you choose products that align with your listening preferences, system balance, and long-term satisfaction." | Approved as the canonical advisor positioning line. Defined as `HOMEPAGE_HEADLINE` in `apps/web/src/app/page.tsx`. Changes equivalent to a positioning shift and require a new decision-log entry. Paired with `HOMEPAGE_SUPPORT` ("Every component evaluated by what it does in your system — not by what it does alone."). |
| 2026-05-08 | Homepage headline refreshed: "Choose components that align with your preferences, system, and long-term happiness." | Supersedes the prior 2026-05-08 lock. Shorter, less self-conscious, drops the "Audio XX helps you" preamble; "happiness" replaces "satisfaction" as more emotionally human. Supporting line demoted to a quieter caption: "Components are evaluated in the context of your system, not in isolation." Same pass reduced hero visual weight (scale −17%, weight 600 → 500, color near-black #151515 → dark charcoal #2A2A2A, measure 42rem → 36rem) and tightened internal/external margins so the page reads as an editorial advisory tool, not a manifesto landing page. |
| 2026-05-08 | Supporting caption removed entirely from the homepage hero | Third pass on the same day. The hero now carries the headline alone — one emotional line, then the interaction surface. The methodology principle the support line carried is implicit in the identity caption above the headline ("System interaction · Listener alignment · Real trade-offs") and explicit downstream in advisory output. `HOMEPAGE_SUPPORT` constant deleted from `apps/web/src/app/page.tsx`. The hero's headline-only composition is the calmest editorial-tool reading achievable without restructuring the page. |
| 2026-05-08 | Homepage headline color softened to charcoal | Fourth pass on the same day. Headline color shifted from the second-pass dark charcoal `#2A2A2A` to `EDITORIAL.inkMuted` (`#3A3A3A`) — the same token used for body prose throughout the app. The hero now sits on the document's calmest tonal level instead of leaning toward near-black, reinforcing the editorial-tool feel. Inline `#2A2A2A` hex removed in favor of the palette token. |
