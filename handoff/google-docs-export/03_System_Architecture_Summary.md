...
Each section opens with a plain-language framing, then expands into specifics. Engineering terms are briefly explained at first mention.
...
| Database | Prisma 6 with libsql / Turso | Prisma is a tool for talking to a database from code. libsql is the database engine; Turso is the hosted service that runs it. |
...
audio-xx/
  ...
  packages/
    ...
    signals/             Phrase-to-trait mappings (how words become signals)

  docs/                  Technical and operational documentation
  handoff/               Onboarding package
  CLAUDE.md              The locked behavioural specification for the advisor
```

The application code lives under `apps/web/`. The curated knowledge lives under `packages/`. The documentation lives under `docs/` and `handoff/`. There is no separate API service or back-end project — the same Next.js application serves both the UI and the reasoning logic.
...
**Plain-language framing.** When a user types a question, the system goes through a sequence of steps to decide what kind of question it is, what listening preferences the words imply, what existing equipment the user owns, and what advisory response is appropriate. The flow always runs the same order. The result is a structured response object that the user interface then draws on screen.
...
Reasoning pipeline         -->  In order:
                                ...
                                4. Frame two or three directional options
...
The five-step framing in the reasoning pipeline (preference extraction, architectural mapping, system-level reasoning, directional framing, restrained conclusion) is the canonical model.
...
- A TypeScript catalog used directly by the advisory engine. Each entry carries axis positions on the four-axis trait framework, trait tendencies, descriptions, retailer links, source citations, and (where calibrated) tendency profiles with confidence scoring.
- A legacy YAML catalog of approximately 127 components in an older qualitative format. Loaded into the database via a seed script.

The two formats are not yet unified. The newer entries live in the TypeScript files; the legacy YAML serves as the database seed and historical reference. Consolidation is on the technical-debt list.

The four-axis sonic trait framework is locked at version 1.
...
The rails currently mount only on the home page route. Other pages (like the systems list or the listener profile) show the top navigation bar only. This is a deliberate scope decision; expanding the rails to all routes is an open architectural question.

The visual style is monochrome editorial — charcoal text on near-white background — with a single restrained brand-red accent used sparingly: the wordmark "XX" glyph, top accent rules, and the active-navigation indicator. All other surfaces are charcoal and cool neutral. There are no warm tints, gradients, shadows, or marketing-style chrome.
...
The application's data is stored in a hosted database (Turso, accessed via Prisma). User authentication is handled by NextAuth with a credentials provider (email and password). Errors that happen in production are reported to Sentry when the appropriate environment variables are populated.

There is no separate "staging" tier. The Vercel previews per branch serve that role. Vercel auto-detects that the project is built with Next.js; no extra configuration file is needed.
...
Practice today is largely manual:
...
- The Vitest suite runs locally before commits

Automation infrastructure that is partially planned:

- A behavioural regression harness — a future test suite designed to drive the canonical questions through the reasoning engine and verify properties of the response (for example: "did the response name a trade-off?", "did the response avoid duplicating the prior turn?"). Specified but not yet built.
- Continuous integration with quality gates — automated checks (type-check, lint, test, security audit) that run before any code can be merged. On the roadmap but not active.
...
Deliberately excluded from this overview, and instead found in the deeper documentation in the repository:

- Per-module implementation detail
- Specific function signatures or code excerpts
- Database schema
