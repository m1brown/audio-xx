...
Quality assurance is shared. The product author retains visual review and prompt-walk responsibility on identity-touching changes. A technical steward retains regression-infrastructure responsibility. Routine quality-assurance gates (type-check preservation, test suite passing) are owned by whichever contributor is making a change.
...
A new technical collaborator joining the project may use AI-assisted tooling at their own discretion within these conventions, or not at all. The conventions are tool-agnostic and apply equally to any coding-capable assistant.

---

## Issue management

Issues live in repository-tracked documentation rather than a separate issue tracker. The project does not currently use GitHub Issues; documentation-as-issue-tracker is sufficient at the current scale.
...
5. Record disposition in the known-issues document
...
- No amending after a pre-commit hook fails — instead, fix the issue and create a new commit
- No skipping of hooks
...
- The project's type-check reports the existing baseline (~98 pre-existing warnings) without new additions
...
- The full quality-assurance checklist
...
- A changelog updated on every push to `main`
...
| Modify the locked behavioural specification or trait framework | Product author only |
| Add a software dependency | Any contributor with justification; notify the product author for non-trivial additions |
| Wire affiliate participation | Product author, with the integrity test mandated by the policy running before deployment |
