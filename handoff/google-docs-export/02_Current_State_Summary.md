...
**Documentation layer.** Substantial documentation exists covering architecture, philosophy, quality-assurance workflow, deployment, and operational model. The behavioural specification and the trait framework documentation are particularly well-developed.
...
**Two parallel systems flows.** A legacy database-backed flow and a newer local-storage-backed flow coexist. Navigation points to the former; the latter is reachable only via direct URL. This is unresolved technical debt.
...
- Image overlay map normalisation pitfalls — products with diacritic characters or hyphen-versus-space variations have caused stealth misses historically
...
- Database hosted on Turso with Prisma
...
Several smaller dashboard-side items also need verification (database backup configuration, Sentry alert routing destinations, domain auto-renewal status).
...
| Tagged releases and changelog | Not in place |
...
**Operational verification gaps.** Several items depend on dashboard inspection (Vercel, Sentry, Turso, domain registrar). These are typically quick checks but have not been completed and recorded.
