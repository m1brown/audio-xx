...
For the deeper version, see [`/docs/ROADMAP.md`](../docs/ROADMAP.md). For the broader milestone framing, see [`/ROADMAP-SPEAKS-FOR-ITSELF.md`](../ROADMAP-SPEAKS-FOR-ITSELF.md).
...
Each is approximately ten minutes of dashboard inspection. The findings should be recorded in the relevant documentation, replacing the current `TODO: verify` markers with confirmed status.

### Manual canonical question walk

Before sharing the project externally with any new reader, a curated list of canonical questions should be walked end-to-end on the live preview environment. The questions are listed in [`/docs/QA_CHECKLIST.md`](../docs/QA_CHECKLIST.md) under "Canonical prompts". Approximately one hour. Bugs surfaced during the walk go on the issue list and are resolved before sharing.
...
Lift the portable reasoning logic into a separate code package within the same repository. The intent is that the reasoning logic — the part that handles preferences, trade-offs, and restraint — could in principle be reused across decision-quality domains beyond audio. The sequence is documented in `/docs/implementation-plan.md` and is gated on the regression test suite, the confidence semantics lock, and an evidence-lineage primitive (a planned feature for explaining why the system reached a given conclusion).
...
- Anything that would compromise the restraint, trade-off, or anti-ranking principles documented in [`/docs/PRODUCT_PHILOSOPHY.md`](../docs/PRODUCT_PHILOSOPHY.md)
