...
The project is in active pre-public-beta development. Concretely:

- The reasoning core is production-quality and is the source of truth on the advisory path
- The visual surface (workspace layout, monochrome editorial palette, restrained accent system) is at a level that does not read as a prototype
- A curated catalog drives advisory output for in-catalog queries
- The project has not yet been shared publicly; current shareable state is a friends-shareable preview environment

The project is not yet at journalist-readiness or public-launch tier. The roadmap to those tiers is documented in [`NEXT_STEPS.md`](NEXT_STEPS.md) and the broader [`/docs/ROADMAP.md`](../docs/ROADMAP.md).
...
- **No live language-model call in the production reasoning path.** Scaffolding exists; runtime call is not wired. The system is fully deterministic today.
...
These limitations are documented in detail in [`CURRENT_STATE_SUMMARY.md`](CURRENT_STATE_SUMMARY.md) and [`/docs/KNOWN_ISSUES.md`](../docs/KNOWN_ISSUES.md).
...
A more detailed roadmap is in [`NEXT_STEPS.md`](NEXT_STEPS.md).
