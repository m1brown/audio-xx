...
export const TECHNOLOGY_PROFILES: TechnologyProfile[] = [
  ...
  // link this page carries.
  {
    ...
    relatedTechnologySlugs: [
      ...
      {
        slug: 'class-a-amplification',
        relation: 'The amplification-side partner sharing the voiced-not-measured posture. NOS argues that the digital path should preserve transient continuity and tonal density through filtering; Class A argues the same for amplification, biasing the active device for full-cycle linear operation rather than letting feedback correct switching artifacts. Both keep a major chain decision visible rather than hiding it inside a spec.',
      },
      {
        slug: 'belt-drive-turntables',
  ...
  // be activated.
  {
    ...
    displayName: 'Step-Up Transformers',
    ...
    figures: [
      {
        ...
        caption: 'An SUT is not an accessory between the cartridge and the phono stage. It is the interface where the cartridge\'s output voltage, source impedance, phono-stage input loading, and transformer construction become one system. The turns ratio determines both voltage gain and the impedance the cartridge sees through the transformer. Nothing here comes free: SUTs give up universality and plug-and-play simplicity for passive gain and cartridge-specific loading that the system builder can voice.',
      },
    ],
  },
    ...
    figures: [
      {
        ...
        caption: 'A high-efficiency loudspeaker is not chosen in isolation. Sensitivity is the architectural commitment that propagates backward through the chain: amplifier choice becomes a voicing decision rather than a power decision, and placement becomes a system parameter rather than an installation detail. The cost is concrete: the speaker gives up bandwidth, ruler-flat response, deep bass extension, compact footprint, and universal placement compatibility in exchange for dynamic ease, microdynamic resolution, and amplifier freedom.',
      },
    ],
  },
  ...
  // are named as the active alternatives, fairly.
  {
    ...
    relatedTechnologySlugs: [
      ...
      {
        slug: 'r2r-dacs',
        relation: 'Sibling digital-source argument. R2R names the conversion architecture as a voiced commitment built from physical resistors; Class A names the amplifier bias as a voiced commitment built from continuous conduction across the cycle. Both pages treat a major chain decision as a voiced choice rather than a number to optimise.',
      },
      {
        slug: 'belt-drive-turntables',
    ...
    figures: [
      {
        ...
        caption: 'Class A is a posture decision before it is a topology decision. The active device is biased to conduct through the entire audio signal cycle — positive and negative excursion alike — rather than switching off across portions of the waveform. The bill comes due in physics: heat, chassis size, and power-per-dollar are forfeit in exchange for tonal continuity through transients and absence of switching-character in the small-signal region. The bias commitment applies equally to single-ended and push-pull, to tube and solid-state, and to power ratings from 1 W (SET) through 200 W (large solid-state Class A) — what unites the cluster is the decision, not the implementation.',
      },
    ],
  },
    ...
    figures: [
      {
        ...
        caption: 'A belt-drive turntable is a mechanical-isolation system in which the belt is one element — combined with platter mass, plinth resonance management, and (in suspended designs) subchassis isolation. The trade-off is concrete: belt-drive gives up the instantaneous torque and pitch precision of direct-drive in exchange for mechanical-noise isolation between motor and platter that the cartridge end of the chain inherits as quietness between notes. The drive-system decision sets the noise floor that everything downstream — cartridge, SUT, phono, amplification, speaker — has to work with.',
      },
    ],
  },
  ...
  // not about device family or commercial era.
  {
    ...
    whyItMatters: 'The Musical Communication School matters because it is the only school of thought in modern audio that operates at the chain level rather than at the component level — and because the chain level is where the educational map the Audio XX Technology Pages now provides comes into focus. Every Technology Page in the corpus describes what one chain junction does. The school page is where the reader learns why those seven junction-level arguments share a coherent posture, why the canonical brands cluster the way they do, and how to recognise a chain expressing the school versus a chain that happens to use some of the same components without the editorial commitment. The page exists so a reader who has finished SET, NOS, R2R, SUTs, High-Efficiency Loudspeakers, Class A, or Belt-Drive Turntables can step up one editorial level and see how those arguments fit into a coherent school — and so the brand pages that name the school in their philosophy now point to an editorial home that holds the school\'s argument together. Other schools of audio exist and are coherent on their own terms. Studio Accuracy treats the recording as the reference and the system\'s job as transparency to it — neutrality, low distortion, and the discipline of a system that adds no character of its own; for a listener who wants to hear what the engineer heard, that is the correct definition of fidelity, and a well-built Studio Accuracy system communicates music exactly by getting out of its way. Engineering-First treats rigorous measured performance as the design discipline that, pursued honestly, yields the most faithful reproduction, and trusts well-understood metrics over editorial voicing precisely because metrics are auditable where taste is not. A listener who holds either position is not making a mistake the Musical Communication School has seen past; they are optimising for a different and defensible definition of fidelity, and the school\'s claim is only that its own definition — chain coherence and musical communication — is worth naming and pursuing, not that the others fail. The Musical Communication School page is the first to be built because its corpus density has reached the threshold at which an editorial map adds clarity rather than authoring clarity that does not yet exist in the underlying work.',
    strengths: [
      ...
      'Editorial argument that survives generations of measurement-target consensus — through the British solid-state high-power era (1970s–1980s), the Japanese digital-CD measurement consensus (1980s–1990s), the modern delta-sigma + Class D efficiency consensus (2000s–2020s), the Musical Communication School has continued to produce, be reviewed, and be bought; the position is durable where fashion-driven design philosophies are not',
