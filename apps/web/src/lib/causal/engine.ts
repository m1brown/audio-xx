...
function composeBlock(
  ...
  return (
    `The ${speaker.name} is a high-efficiency loudspeaker built for low-powered tube ` +
    `amplifiers, and the single-ended-triode ${amp.name} is one. Because the speaker ` +
    `reaches a given listening level with relatively little power, the ${amp.brand} ` +
