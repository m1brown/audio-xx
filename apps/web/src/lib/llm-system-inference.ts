...
/* `verdictFromEvidence` moved to ./relational-explain on 2026-08-24.
 *
 * It is a pure function over established relations and belongs with the other
 * licensing primitives. The move exists so the snapshot layer can compose a
 * verdict without importing THIS module, which carries model prompts and
 * network calls: opening a snapshot must never be able to reassess, and an
 * import of a reasoning module is how that invariant erodes.
 *
 * Re-exported below so existing callers are unaffected.
 */
